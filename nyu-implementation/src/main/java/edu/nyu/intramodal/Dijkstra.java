...
public class Dijkstra {
     ...
     */
    public static <T> HashMap<Map<T, Double>, Map<T, T>> shortestPaths(DirectedGraph<T> graph, T source) {
        /* Create a Fibonacci heap storing the distances of unvisited nodes
         * from the source node.
         */
        FibonacciHeap<T> pq = new FibonacciHeap<T>();

        /* path is a map of each node with its previous node in the shortest path
         * it is defined to find the shortest path
         */
            ...
            /* Update the priorities of all of its edges. */
                ...
                for (Map.Entry<T, Double> arc : graph.edgesFrom(curr.getValue()).entrySet()) {
                   ...
                   if (pathCost < dest.getPriority()){
                       ...
                       path.put(arc.getKey(),curr.getValue());
                   }
...
//        System.out.println(result);
        ...
        HashMap<Map<T, Double>, Map<T, T>> output = new HashMap<>();
        output.put(result, path);
