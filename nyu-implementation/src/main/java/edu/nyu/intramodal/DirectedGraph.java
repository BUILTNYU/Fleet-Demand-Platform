...
public final class DirectedGraph<T> implements Iterable<T> {
     ...
     */
    private final Map<T, HashMap<T,Double>> mGraph = new HashMap<T, HashMap<T, Double>>();
     ...
     *                                do not exist.
     */
    public void addEdge(T one, T two, Double weight) {
        /* Confirm both endpoints exist. */
        if (!mGraph.containsKey(one) || !mGraph.containsKey(two))
            throw new NoSuchElementException("Both nodes must be in the graph.");
        /* Add the edge in both directions. */
        mGraph.get(one).put(two, weight);
//        mGraph.get(two).put(one, weight);
    }
    ...
    public boolean edgeExists(T one, T two) {
        ...
        return mGraph.get(one).containsKey(two);
    }
     ...
     */
    public HashMap<T, Double> edgesFrom(T node) {
        /* Check that the node exists. */
        HashMap<T, Double> arcs = mGraph.get(node);
        if (arcs == null)
     ...
     *
     * @param3 node to test for inclusion.
